...
class Regressor(NeuralNetwork):
    def __init__(self, input_size, hidden_layers, learning_rate=0.001, epochs=100):
...
class Classifier(NeuralNetwork):
    def __init__(self, input_size, hidden_layers, learning_rate=0.01, epochs=100):
    ...
    def forward(self, X):
        ...
        for i in range(len(self.weights)):
            ...
            # activation = self.relu(z)
            if i == len(self.weights) - 1:
                activation = self.sigmoid(z)  #применяется на последнем слое
            else:
                activation = self.relu(z)
